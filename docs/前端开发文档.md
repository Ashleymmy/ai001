...
前端默认访问：`http://localhost:5174`

> 说明：前端会请求后端 `http://localhost:8001`。如果只启动前端不启动后端，设置加载与生成能力会失败。
...
src/
  ...
  features/               # 按领域拆分（agent/editor/project/settings）
...
- Agent v2（灰度路由）：`/agent-v2`、`/agent-v2/:projectId`
- Studio v2（灰度路由）：`/studio-v2`、`/studio-v2/:seriesId`、`/studio-v2/:seriesId/:episodeId`

### 3.1 v2 灰度迁移说明

- v2 页面仅作用于 Studio/Agent，不影响 Home 下四模块、短视频、数字人页面。
- v2 样式在页面根节点使用 `.ui-v2` 作用域，避免污染 `index.css` 的旧样式体系。
- 首页会保留旧入口，同时增加 `Agent v2 Beta` 与 `Studio v2 Beta` 卡片，便于灰度回滚。
- v2 支持查询参数 `stage=<stageId>` 指定初始阶段（仅 UI 层参数，不影响后端 API）。
...
- 组件内会使用类似 `resolveMediaUrl()` 将 `/api/...` 拼成 `http://localhost:8001/api/...`
...
- Electron 打开白屏：确认 Vite 端口为 5174，或检查 `electron/main.js` 的加载地址。
