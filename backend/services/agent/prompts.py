...
3) 单镜头 duration 建议 **5–8 秒**（允许最长 8 秒）；不要把短句拆成独立镜头，2-3 句旁白可以放在一个镜头内。目标：60 秒视频约 8-12 镜头，120 秒视频约 15-20 镜头。
...
6. **音频驱动分镜（重要）**：
   ...
   - 每个镜头的旁白/对白朗读时长必须 **不超过该镜头的 duration**（按自然语速估算，必要时缩短台词/旁白）。
   - 单镜头时长建议 **5–8 秒**（允许最长 8 秒）；不要过度拆分镜头——2-3 句旁白可以归入同一个镜头。60 秒视频约 8-12 个镜头，120 秒约 15-20 个镜头。
7. **总时长约束（如果用户明确给出）**：
